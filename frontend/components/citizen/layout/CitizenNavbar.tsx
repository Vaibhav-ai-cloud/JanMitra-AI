...
import { useSidebar } from "./CitizenSidebar";
    ...
    <header className="sticky top-0 z-20 flex h-16 items-center gap-4 border-b border-white/[0.06] bg-slate-950/90 backdrop-blur-2xl px-4 lg:px-6">
      ...
      <div className="flex-1 max-w-md">
        <div className="relative">
          ...
          <input
            type="search"
            placeholder="Search schemes, complaints..."
            value={searchValue}
        ...
        <div className="relative">
          ...
          >
            <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-gradient-to-br from-blue-500 to-cyan-400 text-xs font-bold text-white">
              {userAvatar ? (
                <Image src={userAvatar} alt={userName} width={28} height={28} className="h-7 w-7 rounded-lg object-cover" />
              ) : (
            ...
            {showProfile && (
              ...
              >
                ...
                {[
                  ...
                  { label: "Settings", icon: Settings, href: "/citizen/settings" },
                  { label: "Help Center", icon: HelpCircle, href: "/citizen/help" },
                ].map((item) => (
                  <Link
                    key={item.href}
                    href={item.href}
